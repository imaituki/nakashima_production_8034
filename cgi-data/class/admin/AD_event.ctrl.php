...
class AD_event {
	...
	function check( &$arrVal, $mode ) {
		...
		$objInputCheck->entryData( "カテゴリー", "event_category", $arrVal["event_category"][0], array( "CHECK_EMPTY_ZERO", "CHECK_MIN_MAX_LEN" ), 0, 255 );
		...
		if( ( strcmp( $mode, "update" ) == 0 ) ) {
			$objInputCheck->entryData( "セミナーID", "all", $arrVal["id_event"], array( "CHECK_EMPTY", "CHECK_NUM" ), null, null );
		}
	...
	function update( $arrVal, $arrSql = null ) {
		...
		unset( $arrVal["end_time"]   );
		// 更新条件
		$where = $this->_CtrTablePk . " = " . $arrVal["id_event"];
		// 更新
	...
	function GetSearchList( $search, $option = null ) {
		...
		if( !empty($search["search_category"]) ){
			$creation_kit["where"] .= " AND event_category = ". $search["search_category"]. " ";
		}
		// 取得条件
		if( empty( $option ) ) {
...
/*
	...
	function GetOption() {
		...
		// SQL配列
		$creation_kit = array(  "select" => "event_category, title",
		...
		if( !empty($arr_option) ){
			foreach( $arr_option as $val ){
				$res[$val["event_category"]] = $val["title"];
			}
		}
