	...
	if( file_exists( $config ) == true ) {
		...
		if( is_array( $_ARR_IMAGE ) ) {
			...
			foreach( $_ARR_IMAGE as $key => $val ) {
				...
				if( !empty( $_FILES["detail"]["tmp_name"][$_POST["_detail_key"]][$val["name"]] ) ) {

					//配列かチェック
					if( is_array( $_FILES["detail"]["tmp_name"][$_POST["_detail_key"]][$val["name"]] ) ){

						foreach ( $_FILES["detail"]["tmp_name"][$_POST["_detail_key"]][$val["name"]] as $key3 => $val3) {

							// 拡張子を取得
							$originImage = array();
							list( $originImage["width"], $originImage["height"], $originImage["extension"] ) = getimagesize( $_FILES["detail"]["tmp_name"][$_POST["_detail_key"]][$val["name"]][$key3] );
							...
							if( $originImage["extension"] < 1 || 3 < $originImage["extension"] ) {
								...
								switch( $originImage["extension"] ) {
									case 1:
										$newImage["imageSrc"]     = imagecreatefromgif( $_FILES["detail"]["tmp_name"][$_POST["_detail_key"]][$val["name"]][$key3] ) or die("errSrcGif!\n");
									...
									break;
									case 2:
										$newImage["imageSrc"]     = imagecreatefromjpeg( $_FILES["detail"]["tmp_name"][$_POST["_detail_key"]][$val["name"]][$key3] ) or die("errSrcJpg!\n");
									...
									break;
									case 3:
										$newImage["imageSrc"]     = imagecreatefrompng( $_FILES["detail"]["tmp_name"][$_POST["_detail_key"]][$val["name"]][$key3] ) or die("errSrcPng!\n");
								...
								// ファイル名の分解
								$pathInfo = pathinfo( $_FILES["detail"]["tmp_name"][$_POST["_detail_key"]][$val["name"]][$key3] . $ext );
								...
								if( is_array( $val["option"] ) ) {
									foreach( $val["option"] as $key2 => $val2 ){
										...
										// 写真リサイズ
										$ImageInfo = $FN_file->resizeImage( array( "inputPath" => $_FILES["detail"]["tmp_name"][$_POST["_detail_key"]][$val["name"]][$key3], "resizeOption" => $val2, "originImage" => $originImage, "newImage" => $newImage ) );
						...
						// 拡張子を取得
						$originImage = array();
						list( $originImage["width"], $originImage["height"], $originImage["extension"] ) = getimagesize( $_FILES["detail"]["tmp_name"][$_POST["_detail_key"]][$val["name"]] );
						...
						if( $originImage["extension"] < 1 || 3 < $originImage["extension"] ) {
							...
							switch( $originImage["extension"] ) {
								case 1:
									$newImage["imageSrc"]     = imagecreatefromgif( $_FILES["detail"]["tmp_name"][$_POST["_detail_key"]][$val["name"]] ) or die("errSrcGif!\n");
									$newImage["extension"]    = "gif";
									$originImage["extension"] = "gif";
									$fn   = "gif";
									$ext  = ".gif";
								break;
								case 2:
									$newImage["imageSrc"]     = imagecreatefromjpeg( $_FILES["detail"]["tmp_name"][$_POST["_detail_key"]][$val["name"]] ) or die("errSrcJpg!\n");
									$newImage["extension"]    = "jpg";
									$originImage["extension"] = "jpg";
									$fn   = "jpeg";
									$ext  = ".jpg";
								break;
								case 3:
									$newImage["imageSrc"]     = imagecreatefrompng( $_FILES["detail"]["tmp_name"][$_POST["_detail_key"]][$val["name"]] ) or die("errSrcPng!\n");
									$newImage["extension"]    = "png";
									$originImage["extension"] = "png";
									$fn   = "png";
									$ext  = ".png";
							...
							// ファイル名の分解
							$pathInfo = pathinfo( $_FILES["detail"]["tmp_name"][$_POST["_detail_key"]][$val["name"]] . $ext );
							...
							if( is_array( $val["option"] ) ) {
								foreach( $val["option"] as $key2 => $val2 ){
									...
									// 写真リサイズ
									$ImageInfo = $FN_file->resizeImage( array( "inputPath" => $_FILES["detail"]["tmp_name"][$_POST["_detail_key"]][$val["name"]], "resizeOption" => $val2, "originImage" => $originImage, "newImage" => $newImage ) );
